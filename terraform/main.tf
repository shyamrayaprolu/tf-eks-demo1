...
module "eks_master_role" {
  ...
  iam_role_name            = "${local.stackname}-${local.environment}-${local.subsystem_val}-eksmaster"
  iam_assume_role_filename = "assumerole-eksmaster-trusted-entities.json"
  iam_role_policy_name     = "${local.stackname}-${local.environment}-${local.subsystem_val}-eksmaster"
...
module "master_securitygroup" {
  ...
  securitygroup_name        = "${local.stackname}-${local.environment}-${local.subsystem_val}-master-sg"
...
module "eks_worker_role" {
  source                   = "./modules/iamrole"
  iam_role_name            = "${local.stackname}-${local.environment}-${local.subsystem_val}-eksworker"
  iam_assume_role_filename = "assumerole-eksworker-trusted-entities.json"
  iam_role_policy_name     = "${local.stackname}-${local.environment}-${local.subsystem_val}-eksworker"
...
module "eks_worker_instprofile" {
  ...
  iam_role_name             = "${local.stackname}-${local.environment}-${local.subsystem_val}-eksworker"
  iam_instance_profile_name = "${local.stackname}-${local.environment}-${local.subsystem_val}-instprofile"
  iaminstprofile_depends_on = [module.eks_worker_role]
}

module "worker_securitygroup" {
  source                    = "./modules/securitygroup"
  securitygroup_name        = "${local.stackname}-${local.environment}-${local.subsystem_val}-worker-sg"
